...
C 
C                                 (February, 1994)
C                     Summary Documentation of Each Routine
C 
...
C 
C 
C                           SUMMARY OF DCDFLIB
C 
C This  library  contains routines  to compute  cumulative  distribution
C functions, inverses, and    parameters  of the  distribution  for  the
C following set of statistical distributions:
C 
C     (1) Beta
C     (2) Binomial
C     (3) Chi-square
C     (4) Noncentral Chi-square
C     (5) F
C     (6) Noncentral F
C     (7) Gamma
C     (8) Negative Binomial
C     (9) Normal
C     (10) Poisson
C     (11) Student's t
C 
C Given values of all but one parameter of a distribution, the other is
C computed.  These calculations are done with  FORTRAN Double Precision 
C variables.
C 
C           -------------------- WARNINGS --------------------
C 
C The F and  Noncentral F distribution are  not necessarily monotone  in
C either degree  of  freedom argument.  Consequently,  there  may be two
C degree of freedom arguments that satisfy the specified condition.  An
C arbitrary one of these will be found by the cdf routines.
C 
C The  amount of computation  required for  the noncentral chisquare and
C noncentral F  distribution    is proportional  to  the  value  of  the
C noncentrality   parameter.  Very large values  of   this parameter can
C require  immense   numbers of   computation.  Consequently,  when  the
C noncentrality parameter is to  be calculated, the upper limit searched
C is 10,000.
C 
C         -------------------- END WARNINGS --------------------
C 
C                             DOCUMENTATION
C 
C This  file  contains an  overview  of the library   and is the primary
C documentation.
C 
C Other documentation  is  in  directory 'doc'  on  the  distribution as
C character  (ASCII) files.  A summary  of all of the available routines
C is contained in dcdflib.chs (chs is an abbreviation of 'cheat sheet').
C The 'chs'  file  will probably be  the  primary reference.  The  file,
C dcdflib.fdoc, contains the  header comments for each  routine intended
C for direct use.
C 
C                              INSTALLATION
C 
C The Fortran source routines are contained in directory src.  
C 
C A  few  routines use   machine  dependent  constants.  Lists  of  such
C constants for different machines are found in ipmpar.f.  Uncomment the
C ones  appropriate to your  machine.  The distributed  version uses the
C IEEE arithmetic that is used by  the IBM PC,  Macintosh, and most Unix
C workstations.
C 
C Ignore compilation warnings that lines of code are  not reachable.  We
C write in a Fortran structured preprocessor (FLECS)  that is similar in
C spirit to  Ratfor.   Sometimes our coding  practices in  FLECS lead to
C unreachable lines.   Also, FLECS inserts   lines of code:  STOP  "CODE
C FLOWING  INTO   FLECS   PROCEEDURES".  All   such    lines should   be
C unreachable.
C 
C                                SOURCES
C 
C The following   routines, written  by   others, are  incorporated into
C DCDFLIB.
C 
C                           Beta Distribution
C 
C DiDinato, A.  R. and Morris, A.  H.   Algorithm 708: Significant Digit
C Computation of the Incomplete Beta  Function Ratios.  ACM Trans. Math.
C Softw. 18 (1993), 360-373.
C 
C                  Gamma Distribution and It's Inverse
C 
C DiDinato, A. R. and Morris, A.  H. Computation of the Incomplete Gamma
C Function  Ratios and  their  Inverse.   ACM  Trans.  Math.   Softw. 12
C (1986), 377-393.
C 
C                          Normal Distribution
C 
C Kennedy and  Gentle, Statistical Computing,  Marcel  Dekker, NY, 1980.
C The rational function approximations  from pages 90-95 are used during
C the calculation of the inverse normal.
C 
C Cody, W.D.  (1993).  "ALGORITHM  715:  SPECFUN  -  A Portabel  FORTRAN 
C Package   of  Special  Function   Routines   and  Test  Drivers",  acm
C Transactions on Mathematical Software. 19, 22-32.  A slightly modified
C version of Cody's function  anorm  is used for the cumultive normal.
C 
C                              Zero Finder
C 
C J.   C. P.   Bus and  T.  J.  Dekker.   Two Efficient  Algorithms with
C Guaranteed Convergence  for Finding a  Zero of a Function.  ACM Trans.
C Math. Softw. 4 (1975), 330.
C 
C We transliterated Algoritm R of this paper from Algol to Fortran.
C 
C                           General Reference
C 
C Abramowitz,  M. and Stegun,  I. A.  Handbook of Mathematical Functions
C With  Formulas, Graphs,  and   Mathematical Tables.   (1964)  National
C Bureau of Standards.
C 
C This book has been reprinted by Dover and others.
C 
C 
C                               LEGALITIES
C 
...
C
C                     HOW TO USE THE ROUTINES
C 
C The calling sequence for each routine is of the form:
C 
C   SUBROUTINE CDF<name>( WHICH, P, Q, X, <parameters>, STATUS, BOUND ).
C 
C WHICH and STATUS are INTEGER, all other arguments are DOUBLE PRECISION.
C 
C <name> is a one to  three character name identifying the distribution.
C WHICH  is an input integer value  that identifies what parameter value
C is to be calculated from the values of the other parameters.
C 
C P is always the cdf evaluated at X, Q is always the compliment of the
C cdf evaluated at X, i.e.  1-P, and X is always the value at which the
C cdf  is evaluated.   The auxiliary parameters,  <parameters>,  of the
C distribution differ by distribution.
C 
C If WHICH is 1, P and  Q are to be calculated, i.e., the cdf; if WHICH
C is 2, X is to be calculated, i.e., the inverse cdf.  The value of one
C auxiliary parameter in <parameters> can also be the value calculated.
C 
C STATUS returns 0 if the calculation completes correctly.
C 
C            --------------------WARNING--------------------
C 
C If STATUS is not 0, no meaningful answer is returned.
C 
C         -------------------- END WARNING --------------------
C 
C STATUS returns  -I if the I'th  input parameter was  not  in the legal
C range (see below).  Parameters are counted  with WHICH being the first
C in these return values.
C 
C A STATUS  value of 1 indicates that  the desired answer was apparently
C lower than the lower bound on the search interval.  A return code of 2
C indicates that  the answer was  apparently higher than the upper bound
C on the search interval.  A return code of 3 indicates that P and Q did
C not sum to 1. Other positive codes are routine specific.
C 
C BOUND is not  set if STATUS is returned  as 0.  If  STATUS is -I  then
C BOUND is   the bound illegally  exceeded by  input  parameter I, where
C WHICH  is  counted as 1,  P as 2,  Q as 3,  X as 4, etc.  If STATUS is 
C returned as 1 or 2 then BOUND  is returned as the lower or upper bound
C on the search interval respectively.
C 
C                                 BOUNDS
C 
C Below are  the rules that we used  in determining bounds on quantities
C to be  calculated.   Those who don't care   can find a summary  of the
C bounds in  dcdflib.chs.   Input bounds  are  checked for  legality  of
C input.  The search  range  is  the range   of values searched  for  an
C answer.
C 
C                              Input Bounds
C 
C Bounds on input parameters are  checked by the  CDF* routines.   These
C bounds were set according to the following rules.
C 
C P: If the  domain of the cdf (X) extends to  -infinity  then P must be
C greater than 0 otherwise P must be greater than or equal to 0.  P must
C always be less than or equal to 1.
C 
C Q: If the  domain of the cdf (X) extends to  +infinity  then Q must be
C greater than 0 otherwise Q must be greater than or equal to 0.  Q must
C always be less than or equal to 1.
C 
C Further, P and Q must sum to 1. The smaller of the two P and Q will be
C used in calculations to increase accuracy
C 
C X:  If  the  domain is infinite  in   either the positive  or negative
C direction, no check  is performed in  that direction.  If the left end
C of the domain is 0, then X is checked to assure non-negativity.
C 
C DF, SD, etc.:  Some auxiliary parameters must  be positive. The lowest
C input values accepted for these parameters is 1E-300.
C 
C 
C                                 Search Bounds
C 
C These are the  ranges searched for an  answer.   If the domain  of the
C parameter in the cdf  is closed at  some  finite value, e.g., 0,  then
C this value is the same endpoint of the search range.  If the domain is
C open  at  some finite   endpoint (which only  occurs   for  0 --  some
C parameters must be strictly positive) then  the endpoint is 1E-300. If
C the  domain is infinite in either  direction then +/- 1E300 is used as
C the endpoint of the search range.
C 
C                         HOW THE ROUTINES WORK
C 
C The cumulative  distribution   functions are computed  directly.   The
C normal, gamma,  and  beta functions use the  code  from the references
C cited.  Other  cdfs are calculated  by relating them  to one  of these
C distributions.  For example, the  binomial and negative binomial  cdfs
C can be converted  to a beta cdf.   This is how fractional observations
C are handled.  The  formula from Abramowitz  and Stegun  for converting
C the cdfs is cited  in the fdoc file.    (We think the formula  for the
C negative binomial in A&S is wrong, but there is a correct one which we
C used.)
C 
C The inverse normal and gamma are also taken  from the references.  For
C all other parameters, a search is made for the value that provides the
C desired P.  Initial  values are chosen crudely  for the search  (e.g.,
C 5).  If the domain  of the cdf for the  parameter being calculated  is
C infinite, a step doubling strategy is  used to bound the desired value
C then the  zero  finder is  employed  to refine the answer.    The zero
C finder attempts to obtain the answer accurately to about eight decimal
C places.
C 
C 
