...
c     Passed Variables
      Real*4 rvec(*)
      Integer*4 lenv, ntotin, ntot2n, ijklut, ntotut, ntot2t
c
c     Local Variables
      Integer*4 ntot, ntot2, ijkl, kalled, ijklin, ij, kl, i, j,
     $     k, l, ii, jj, m, i24, loop2, now, idum, ivec
      Real*4 cd, cm, twom24, s, t, uni, zuni
c
c     Common block
      Real*4 u, c
      Integer*4 i97, j97
...
C          always come here to initialize
    1 CONTINUE
      ...
      K = MOD(KL/169, 178) + 1
      L = MOD(KL, 169)
CCC      WRITE(6,'(A,I10,2X,2I10)') ' RANMAR INITIALIZED:',IJKL,NTOT,NTOT2
...
C            (NTOT2*MODCNS + NTOT) random numbers
      ...
      IF (NOW .GT. 0)  THEN
CCC        WRITE(6,'(A,I15)') ' RMARIN SKIPPING OVER ',NOW
       DO 40 IDUM = 1, NTOT
       UNI = U(I97)-U(J97)
